...
const modifiedPrims = (maze: Maze) => {
  ...
  while (queue.length > 0 && count > 0) {
    ...
    const possibleRoutes = currentNeighbors.filter((cell) =>
      mazeContains(cell)
    ) as Array<Cell>;
    count--;

    const currentNeighborsActual = Object.values(currentNode.neighbors).filter(
      (neighbor) => {
        return neighbor !== null;
      }
    );

    console.log(currentNeighborsActual);

    // if only one of it's neighbors is in the maze
    if (possibleRoutes.length > 0 && currentNeighborsActual.length < 1) {
